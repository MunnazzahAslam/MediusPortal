...
function AllCity() {
    ...
    var columns = [
        {
            title: "CITY #", field: "id", hidden: false, filterPlaceholder: '1', editable: 'never',
            render: rowData =>
                <Link to={`/City/${rowData.id}`}>
                    <p style={{ color: '#05a677', fontWeight: 'bold' }}>{rowData.id}</p>
                </Link>
        },
        {
            title: "NAME", field: "name", hidden: false, filterPlaceholder: 'Sukkur', editable: 'never', cellStyle: {
                textAlign: 'left'
            },
            render: rowData =>
                <p>{rowData.name}</p>
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        api.put("/api/City/Update", newData)
            .then(res => {
    ...
    return (
        <div>
            <div><h3 className="mt-lg-4" style={{ color: '#262B40' }}>Cities Details</h3>
                <p style={{ color: '#4a5073' }}>All the cities are listed below and you can add, edit, or delete Intellectual Property Cities.</p></div>
