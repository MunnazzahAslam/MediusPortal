...
function AllCopyrights() {

  var columns = [
    {
      title: "TITLE", field: "title", hidden: false, filterPlaceholder: 'Pizza 2000 Copyright', editable: 'never', cellStyle: {
        textAlign: 'left'
      }, width: 30,
      render: rowData =>
  ...
  useEffect(() => {
    api.get("/api/Case/GetAllCopyright")
      .then(res => {
        setData(res.data)
      })
      .catch(error => {
  ...
  return (
    <div>
      <div><h3 className="mt-lg-4" style={{ color: '#262B40' }}>Copyrights Cases</h3>
        <p style={{ color: '#4a5073' }}>All the copyrights related cases are in the below table and you can click on each case to view its details.</p></div>
